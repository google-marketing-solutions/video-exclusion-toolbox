...
resource "google_cloudfunctions2_function" "gads_channel_report_fetcher" {
  location    = var.region
  name        = "vet-gads-channel-report-fetcher"
  ...
  build_config {
    ...
    source {
      storage_source {
        bucket = google_storage_bucket.source_archive.name
        object = google_storage_bucket_object.gads_channel_report_fetcher.name
      }
    }
  }
...
resource "google_cloudfunctions2_function" "yt_channel_fetcher" {
  location    = var.region
  name        = "vet-yt-channel-fetcher"
  description = "Pull YouTube channel metadata for placements in Google Ads reports."
  ...
  service_config {
    ...
    environment_variables = {
      GOOGLE_CLOUD_PROJECT      = var.project_id
      VID_EXCL_BIGQUERY_DATASET = google_bigquery_dataset.video_exclusion_toolbox.dataset_id
      VET_BIGQUERY_SOURCE_TABLE = google_bigquery_table.google_ads_report_channel.table_id
      VET_BIGQUERY_TARGET_TABLE = google_bigquery_table.youtube_channel.table_id
    }
    ingress_settings               = "ALLOW_INTERNAL_ONLY"
    service_account_email          = google_service_account.video_exclusion_toolbox.email
    all_traffic_on_latest_revision = true
  ...
  build_config {
    ...
    source {
      storage_source {
        bucket = google_storage_bucket.source_archive.name
        object = google_storage_bucket_object.yt_channel_fetcher.name
      }
    }
  }
  ...
  event_trigger {
    trigger_region        = var.region
    event_type            = "google.cloud.pubsub.topic.v1.messagePublished"
    pubsub_topic          = google_pubsub_topic.youtube_channel.id
    retry_policy          = "RETRY_POLICY_RETRY"
    service_account_email = google_service_account.video_exclusion_toolbox.email
  }
