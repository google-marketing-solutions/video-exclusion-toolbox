...
YTAD_YOUTUBE_CHANNEL_PUBSUB_TOPIC = os.environ.get(
    'YTAD_YOUTUBE_CHANNEL_PUBSUB_TOPIC'
)
...
def start_job(
  ...
  report_df = get_report_df_channels(customer_id, lookback_days, gads_filters)
...
def write_results_to_gcs(report_df: pd.DataFrame, customer_id: str) -> str:
  ...
  Returns:
      The blob_name written to or an empty string.
  """
  ...
  if number_of_rows > 0:
    blob_name = f'google_ads_report_channel/{customer_id}_{today}.csv'
...
def send_messages_to_pubsub(customer_id: str, blob_name: str) -> None:
  ...
  pubsub.send_dict_to_pubsub(
      ...
      topic=YTAD_YOUTUBE_CHANNEL_PUBSUB_TOPIC,
