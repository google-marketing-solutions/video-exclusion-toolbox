...
"""Fetch Google Ads channel placement reports and persist new records to BigQuery."""
...
BIGQUERY_TABLE_NAME = 'google_ads_report_channel'
...
def get_report_query(
  ...
  """Builds and returns the GAQL query for channel placement reporting."""
...
def _fetch_and_stream_report_records(
  ...
  """Queries Google Ads and streams channel records directly into an in-memory NDJSON buffer.
  ...
  Returns:
      A tuple of (ndjson_buffer, retrieved_channel_ids_set, total_row_count).
  """
  clean_customer_id = sanitize_gads_id(customer_id)
  logger.info(
      '[%s] Querying Google Ads channel report stream...', clean_customer_id
  )
    ...
    try:
      ...
      for batch in stream:
        for row in batch.results:
          gp = getattr(row, 'group_placement_view', None) or getattr(
              row, 'detail_placement_view', None
          )
          if not gp:
            continue
          m = row.metrics
          channel_id = gp.placement.strip() if gp.placement else ''
          if not channel_id:
            continue
          ...
          campaign_name = ''
          if hasattr(row, 'campaign') and row.campaign and row.campaign.name:
            campaign_name = str(row.campaign.name)

          ad_group_obj = getattr(row, 'ad_group', None)
          ad_group_id = 0
          if ad_group_obj and hasattr(ad_group_obj, 'id') and ad_group_obj.id:
            ad_group_id = int(ad_group_obj.id)
          ad_group_name = ''
          if (
              ad_group_obj
              and hasattr(ad_group_obj, 'name')
              and ad_group_obj.name
          ):
            ad_group_name = str(ad_group_obj.name)

          placement_type_str = (
              gp.placement_type.name
              if hasattr(gp.placement_type, 'name')
              else str(gp.placement_type)
          )
          ...
          raw_views = getattr(
              m, 'video_trueview_views', getattr(m, 'video_views', 0)
          )
          raw_view_rate = getattr(
              m, 'video_trueview_view_rate', getattr(m, 'video_view_rate', 0.0)
          )

          placement_target_url = str(
              getattr(gp, 'target_url', '')
              or getattr(gp, 'group_placement_target_url', '')
              or ''
          )
          youtube_channel_name = str(getattr(gp, 'display_name', '') or '')
          ...
          if composite_key in records_by_key:
            ...
            existing['video_views'] += int(raw_views)
            existing['clicks'] += int(m.clicks)
            if youtube_channel_name and not existing['youtube_channel_name']:
              existing['youtube_channel_name'] = youtube_channel_name
            if placement_target_url and not existing['placement_target_url']:
              existing['placement_target_url'] = placement_target_url
          else:
      ...
      for record in records_by_key.values():
        ...
        if clicks > 0:
          record['all_conversions_from_interactions_rate'] = float(
              conv
          ) / float(clicks)
        else:
          record['all_conversions_from_interactions_rate'] = 0.0
      ...
      buffer.seek(0)
      retrieved_channel_ids = {k[3] for k in records_by_key.keys()}
      row_count = len(records_by_key)
      return buffer, retrieved_channel_ids, row_count
      ...
      if is_transient and attempt < max_retries:
        ...
        logger.error(
            '[%s] Failed to fetch Google Ads channel report: %s',
            clean_customer_id,
            ex,
        )
        raise
      ...
      logger.error(
          '[%s] Unexpected error querying Google Ads channel report: %s',
          clean_customer_id,
          ex,
      )
      raise
...
def run(
  ...
  """Fetches channel placements, upserts today's partition via MERGE, and notifies downstream."""
  telemetry = telemetry or PipelineTelemetryContext(
      logger=logger,
      service_name=os.environ.get(
          'K_SERVICE', 'vet-gads-channel-report-fetcher'
      ),
      customer_id=customer_id,
  )
  logger.info(
      '[%s] Starting channel placement report pipeline (lookback: %d days).',
      customer_id,
      lookback_days,
  )
  buffer, retrieved_channel_ids, row_count = _fetch_and_stream_report_records(
      customer_id=customer_id,
      lookback_days=lookback_days,
      gads_filters=gads_filters,
  )
  ...
  telemetry.log_step(
      ...
      metadata={
          'unique_channel_ids_count': len(retrieved_channel_ids),
          'lookback_days': lookback_days,
      },
  )
  ...
  if row_count == 0:
    logger.info(
        '[%s] No channel placements returned from Google Ads.', customer_id
    )
    telemetry.log_step(
        step='SKIPPED',
        records_out=0,
  ...
  logger.info(
      '[%s] Retrieved %d placements (%d unique channel IDs) from Google Ads.',
      customer_id,
      row_count,
      len(retrieved_channel_ids),
  )
  ...
  new_channel_ids = retrieved_channel_ids - existing_ids
  logger.info(
      "[%s] Discovered %d net-new channel placements for today's partition"
      ' (%s).',
      customer_id,
      len(new_channel_ids),
      today_date,
  ...
  telemetry.log_step(
      ...
      records_in=len(retrieved_channel_ids),
      records_out=len(new_channel_ids),
      metadata={
          'partition_date': str(today_date),
          'existing_ids_count': len(existing_ids),
          'net_new_ids_count': len(new_channel_ids),
      },
  )
  ...
  upsert_ndjson_to_bq(
      ...
      dataset_id=BIGQUERY_DATASET,
      table_name=BIGQUERY_TABLE_NAME,
      key_columns=['customer_id', 'campaign_id', 'ad_group_id', 'channel_id'],
      partition_date=today_date,
  ...
  if new_channel_ids:
    publish_batch(
        project_id=GOOGLE_CLOUD_PROJECT,
        topic_id=YOUTUBE_CHANNEL_PUBSUB_TOPIC,
        messages=[
    ...
    logger.info(
        '[%s] Published delta notification to %s for %d new channels.',
        customer_id,
        YOUTUBE_CHANNEL_PUBSUB_TOPIC,
        len(new_channel_ids),
    )
    telemetry.log_step(
        step='PUBLISH_DOWNSTREAM',
        records_in=len(new_channel_ids),
        records_out=1,
        metadata={
    ...
    logger.info(
        '[%s] All %d placements updated in BigQuery via MERGE; no new channels'
  ...
  logger.info('[%s] Completed channel placement report pipeline.', customer_id)


@functions_framework.cloud_event
def main(cloud_event: functions_framework.cloud_event) -> None:
  """CloudEvent entrypoint for Google Ads channel placement reporting."""
  ...
  try:
    ...
    logger.error(
        '[%s] Error executing channel placement reporting pipeline: %s',
        customer_id,
        e,
        exc_info=True,
    )
